...
proto.sliceView = function(matrix) {
	const part = matrix.slice(this.position.y, this.dimension.height + this.position.y)
	const self = this
	return part.map((row) => {
		return row.slice(self.position.x, self.dimension.width + this.position.x)
	})
}

proto.render = function() {
	this.dirty = false
	const slice = this.sliceView(this._preRendered)
...
const createTable = (options) => {
	...
	table.on('keypress', function (key, e) {
		...
		if(renderer.dirty) {
			//screen.log('redrawing: ' , renderer.position)
			table.setContent(renderer.render())
			screen.render()
		}
	})
